...
| **B. `multi_search([q…])` — batch query embeddings** ✅ SHIPPED | **no-recompute bulk** | one embed call for N queries; **~4–5× warm on 8–10 queries**, batched embeddings numerically identical to single-query (row-cosine 1.0), results identical. Recompute/temporal/grep/pure-BM25 fall back to per-query. (Initial ship was a no-op — a leftover reset clobbered the embedding; codex caught it, fixed.) | S | LEANN |
