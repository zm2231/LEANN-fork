...
| Field | Type | Required | Example | Notes |
|---|---|---|---|---|
| `source_type` | enum string | yes | `"document"`, `"git_commit"`, `"slack"`, `"email"`, `"imessage"`, `"browser_history"`, `"calendar"`, `"code"`, `"voice_memo"`, `"journal"` | Open enum; new sources add new values. Used for source-typed filtering and per-type extraction rules. |
