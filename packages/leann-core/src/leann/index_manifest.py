...
``leann list --refresh`` rebuilds the manifest from a full disk scan, so a
stale or pre-manifest install self-heals on demand (and automatically on the
first list, when the manifest is empty).
