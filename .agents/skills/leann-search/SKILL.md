...
CLI/MCP entry points + Python API. Embeddings flow through iq at `http://100.122.112.83:8100/v1` (Tailscale, works from any device).
...
| Agent/tool search over MCP | `leann_search` / `leann_multi_search` |
...
# Hybrid/exact routing and sparse-filter diagnostics
leann search my-docs "OrderHandler timeout" \
  --vector-weight 0.4 \
  --metadata-filters '{"source_type": {"==": "code"}}' \
  --prefilter auto \
  --explain-filters \
...
- `--prefilter auto|always|never` — sparse-filter routing (default `auto`)
- `--prefilter-threshold FLOAT` — selectivity cutoff for `auto` (default `0.05`)
- `--explain-filters` — include filter routing diagnostics (`--json` wraps output as `{results, diagnostics}`)
- `--diversify-by FIELD` — cap results by a metadata field
- `--max-per-group N` — per-group cap for `--diversify-by` (default 2)
...
`index_name` and `query`; these search controls are optional: `top_k`,
`complexity`, `metadata_filters`, `vector_weight`, `prefilter`,
`prefilter_threshold`, `explain_filters`, `diversify_by`, and
`max_per_group`. The MCP handler also accepts camelCase aliases for the new
